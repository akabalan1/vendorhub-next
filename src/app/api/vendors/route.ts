...
export async function GET(req: NextRequest) {
  ...
  const data = vendors.map(v => {
    ...
    v.feedback.forEach(f=>{ if(f.ratingQuality!=null)ratings.push(f.ratingQuality); if(f.ratingSpeed!=null)ratings.push(f.ratingSpeed); if(f.ratingComm!=null)ratings.push(f.ratingComm); });
...
export async function POST(req: NextRequest) {
  ...
  const { name, overview, platforms = [], industries = [], costTiers = [], capabilities = [] } = body || {};
  if (!name) return NextResponse.json({ error: 'name required' }, { status: 400 });
  const vendor = await prisma.vendor.create({ data: { name, overview, platforms, industries } });
  ...
  if (Array.isArray(capabilities) && capabilities.length) {
    // upsert capabilities by slug, then link
    for (const c of capabilities) {
      const cap = await prisma.capability.upsert({ where: { slug: c.slug }, create: { slug: c.slug, name: c.name ?? c.slug }, update: {} });
